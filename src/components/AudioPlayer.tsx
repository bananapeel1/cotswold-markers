...
export default function AudioPlayer({
  ...
  return (
    <div className="flex items-center gap-4 bg-surface-container rounded-full px-5 py-3 mt-4">
      <button
          ...
          <div className="flex-1 h-1 bg-surface-variant rounded-full overflow-hidden">
            <div
              className={`h-full bg-primary rounded-full ${
                playing ? "animate-[grow_90s_linear]" : ""
              }`}
              style={{ width: playing ? "0%" : "0%" }}
            />
          </div>
          <span className="text-[10px] text-secondary font-mono">{duration}</span>
        </div>
      </div>
    </div>
  );
